...
\begin{align}
    ...
    \frac{\mathrm{d}Y_k}{\mathrm{d}t} &= \theta_k \frac{a_1 Y_2 Y_5}{k_2 + Y_2} - bY_3 Y_k \quad (k=1,2,3).
\end{align}
...
\begin{align}
    \lambda &= b - 2Ab^{3/2} +O(b^2), \\
    \text{Small} &= A\sqrt{b} - g(a_1, a_2; k_1, k_2)b.
\end{align}
The small and large molecule fractions are invariant under exchange of indices. The growth rate is parabolic in ribosomal content in the leading order: $ \lambda = \frac{a_2}{k_2}R^2 + O(R^4) $.

\subsubsection{Overabundance of Nutrient}
In the opposite limit, we can supply the cell with high levels of nutrient, such that $b/a_1, b/a_2 \gg 1$. In this limit, numerical experiments tell us that unexpected behaviors emerge. This section will analyze the relevant dependencies.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{(a) Growth rate is approximately linear in $b$ with slope $1$. Blue line is from theory. (b) The metabolic/housekeeping fraction is approximately constant. (c) Growth rate is quadratic in $b$ near $0$, and appears to be linear for higher $b$. (d) Growth rate with excessive nutrient.}
    \label{fig:b_dependence}
\end{figure}

During starvation, more resources is allocated to transportation, while in excessive nutrient, the cell has reached its \textit{optimal allocation strategy} and can no longer increase its efficiency: figures \ref{fig:theta_b_1} and \ref{fig:theta_b_2}.
\begin{figure}
    \centering
    \begin{subfigure}[t]{0.5\linewidth}
        \centering
...
In the high nutrient limit, figure \ref{fig:high_b} shows that the metabolic sector fraction is again nearly constant, as assumed in \cite{Scott2010}. The analytic value of this constant fraction is topic of future discussion. 
...
We choose a decreasing positive sequence of $(a_j, k_j)$ to mimic the fact that metabolism usually becomes progressively slower down the reaction pathway. 

Intermediate $b$ value section that trajectories and growth rate may be non-monotonic. See figure \ref{fig:medium_b}.
\begin{figure}
    \centering
    \begin{subfigure}[t]{0.5\linewidth}
        \centering
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{(a) When nutrient level rises, behavior of the $\lambda$-$b$ curve with six partitions changes qualitatively. (b) One of the metabolic sectors with six sectors. The fraction is approximately constant. }
    \label{fig:more_partitions}
\end{figure}

\section{Ongoing Work}
Study the overabundance and bottleneck reaction limits theoretically. Analyze in the Lagrangian view using waiting time of the ODEs, as proposed by Wei-Hsiang Lin \cite{lin2025biomasstransferautocatalyticreaction}. Study the correlation of protein fractions in the $n$ metabolic sectors, given a positive sequence $(a_j, k_j)$. Michaelis-Menten kinetics. Reconcile quadratic growth rate with the linear relationship observed in experiments. Conduct analytical error analysis.
